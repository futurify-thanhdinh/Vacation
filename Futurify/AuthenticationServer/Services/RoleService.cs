...
namespace AuthenticationServer.Services
{
    public class RoleService : IRoleService
    {
        ...
        {
            var existRole = _context.Roles.FirstOrDefault(r => r.Id == roleId);
            ...
            var existPermission = _context.Permissions.FirstOrDefault(p => p.Id.ToLower() == permissionId.ToLower());
        ...
        public async Task AssignPermissionsAsync(int roleId, string[] permissionCodes)
        {
            ...
            var existPermissions = _context.Permissions.Where(p => addPermissions.Contains(p.Id)).ToList();
        ...
        {
            var existRole = _context.Roles.FirstOrDefault(r => role.Id == r.Id);
            if (existRole == null)
            {
                throw new CustomException(Errors.ROLE_NOT_FOUND, Errors.ROLE_NOT_FOUND_MSG);
            }

            if (await IsDuplicatedRoleNameAsync(role.Name, role.Id))
            {
                throw new CustomException(Errors.ROLE_DUPLICATED_NAME, Errors.ROLE_DUPLICATED_NAME_MSG);
            }
        ...
        public async Task<bool> IsDuplicatedRoleNameAsync(string name, int? id)
        {
            if (id.HasValue)
            {
                return await _context.Roles.AnyAsync(t => t.Id != id.Value && t.Name.ToLower() == name.ToLower());
            }
            else
            {
